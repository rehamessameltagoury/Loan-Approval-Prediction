...
def run_data_prep(
    raw_data_path: str, dest_path: str, filename: str = "loan_approval_dataset"
):
    # Load parquet files
    df = read_dataframe(os.path.join(raw_data_path, f"{filename}.csv"))
...
def run_register_model(
    data_path: str, top_n: int, HPO_EXPERIMENT_NAME, EXPERIMENT_NAME
):
    client = MlflowClient()
